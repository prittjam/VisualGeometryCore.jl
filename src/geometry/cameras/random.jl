...
"""
    sample_p3p(model::CameraModel{PhysicalIntrinsics}, X, sensor_bounds; max_retries=1000)
    sample_p3p(rng, model::CameraModel{PhysicalIntrinsics}, X, sensor_bounds; max_retries=1000)

Sample P3P configuration for camera with PhysicalIntrinsics using physical sensor coordinates.

This method generates correspondences in physical units (mm) by:
1. Sampling 3 random pixel coordinates from sensor_bounds
2. Unprojecting them to the CMOS sensor plane at depth = focal_length
3. This gives 3D sensor coordinates in mm (not just 2D pixel coordinates)

# Arguments
- `rng=Random.default_rng()`: Random number generator (optional)
- `model::CameraModel{PhysicalIntrinsics}`: Camera model with physical parameters
- `X::Vector{<:StaticVector{3,Float64}}`: 3D world points in mm (at least 3 points)
- `sensor_bounds::Rect`: Sensor bounds in pixels for sampling

# Keyword Arguments
- `max_retries=1000`: Maximum sampling attempts

# Returns
- `(extrinsics, csponds)`: Tuple containing:
  - `extrinsics::Vector{EuclideanMap}`: Valid camera extrinsics from P3P
  - `csponds::StructArray{Cspond}`: 3D world point (mm) → 3D sensor point (mm) correspondences
...
# 3D world points in mm
X = [SVector(100.0, 100.0, 0.0), SVector(200.0, 150.0, 0.0), ...]

# Sample P3P with physical sensor coordinates
extrinsics, csponds = sample_p3p(model, X, ustrip(Rect(sensor)))

# Correspondences are in mm:
# csponds.source: 3D world points (mm)
# csponds.target: 3D sensor points (mm) on CMOS at focal length
```
"""
function sample_p3p(model::CameraModel{<:PhysicalIntrinsics},
                    X::Vector{<:StaticVector{3,Float64}},
                    sensor_bounds::Rect;
                    max_retries::Int=1000)
    return sample_p3p(Random.default_rng(), model, X, sensor_bounds; max_retries=max_retries)
...
function sample_p3p(rng::Random.AbstractRNG,
                    model::CameraModel{<:PhysicalIntrinsics},
                    X::Vector{<:StaticVector{3,Float64}},
                    sensor_bounds::Rect;
                    max_retries::Int=1000)
    ...
    # Get focal length for unprojection depth
    f = focal_length(model.intrinsics)

    for attempt in 1:max_retries
        # Sample 3 random 3D world points
        sampled_idx = randperm(rng, length(X))[1:3]
        X3 = X[sampled_idx]

        # Sample 3 random pixel coordinates in sensor bounds
        u3_pixels = rand(rng, sensor_bounds, 3)

        # Unproject to CMOS sensor plane at focal length to get physical coordinates (mm)
        sensor_points_mm = [SVector{3,Float64}(unproject(model, SVector{2,Float64}(u), f))
                           for u in u3_pixels]

        # Create correspondences: 3D world (mm) → 3D sensor (mm)
        # Both are now in physical units
        csponds = StructArrays.StructArray{Cspond{eltype(X3), eltype(sensor_points_mm)}}((X3, sensor_points_mm))

        # Try P3P with physical sensor coordinates
        try
            Rs, ts = p3p(model, csponds)
            ...
            # P3P can fail for invalid configurations
            continue
        end
    end
