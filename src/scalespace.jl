...
"""
struct ScaleOctave
    G::Array{Gray{Float32},3}        # 3D Gaussian cube (H^o, W^o, Lg)
    ...
    step::Float64                    # Pixel stride (2^octave, can be fractional for upsampled octaves)
end
...
function ScaleSpace(first_octave::Int, last_octave::Int, octave_resolution::Int,
    ...
    for o in octave_range
        if o == first(octave_range)
            ...
            H_o = length(1:2:size(prev_octave.G, 1))
            W_o = length(1:2:size(prev_octave.G, 2))
        end
    ...
    for (i, (o, s)) in enumerate(zip(octaves, subdivisions))
        octave_obj = octave_objects[findfirst(oct -> oct.octave == o, octave_objects)]
        first_sub = first(subdivision_range)
        slice_idx = (s - first_sub) + 1
        view_data = @view octave_obj.G[:, :, slice_idx]
