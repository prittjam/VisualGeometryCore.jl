...
"""
    refine_extremum_3d_manual(octave_3d::Array{T,3}, x::Int, y::Int, z::Int,
                              octave_num::Int, first_subdivision::Int, octave_resolution::Int,
                              base_scale::Float64, step::Float64) where T

Original version using manual derivative computation (for comparison/testing).
...
1. Computes gradient ∇f and Hessian H at current **integer position** using finite differences
   - Second derivatives: `Dxx = f(x+1) + f(x-1) - 2f(x)`
   - Mixed derivatives: `Dxy = 0.25 * (f(x+1,y+1) + f(x-1,y-1) - f(x-1,y+1) - f(x+1,y-1))`
   - Note: Always evaluated at discrete grid points, never at sub-pixel locations
2. Solves H·b = -∇f for sub-pixel offset vector b (quadratic fit peak)
3. If |b[i]| > 0.6, moves to adjacent **integer** pixel and re-fits
   - Takes integer steps: dx, dy ∈ {-1, 0, 1}
   - Sub-pixel refinement comes from the final offset b, not from fractional positions
...
- [`detect_extrema`](@ref): High-level detection interface
"""
function refine_extremum_3d_manual(octave_3d::Array{T,3}, x::Int, y::Int, z::Int,
                                  octave_num::Int, first_subdivision::Int, octave_resolution::Int,
                                  base_scale::Float64, step::Float64) where T
    ...
    for iter in 1:max_iterations
        # Update position by integer steps (dx, dy, dz ∈ {-1, 0, 1})
        x += dx
        y += dy
        z += dz
        ...
        # Access HESSIAN DETERMINANT RESPONSE at current INTEGER position
        # Note: octave_3d contains the pre-computed Hessian determinant response
        # We're now computing derivatives OF THAT RESPONSE to fit a 3D paraboloid
        at(ddx, ddy, ddz) = Float64(octave_3d[y+ddy, x+ddx, z+ddz].val)

        # Compute gradient of RESPONSE using central differences
        Dx = 0.5 * (at(1, 0, 0) - at(-1, 0, 0))
        Dy = 0.5 * (at(0, 1, 0) - at(0, -1, 0))
        Dz = 0.5 * (at(0, 0, 1) - at(0, 0, -1))

        # Compute Hessian of RESPONSE using second differences
        # This gives us the curvature of the response surface
        val_center = at(0, 0, 0)
        ...
        # Solve H·b = -g to find peak of fitted 3D paraboloid (quadratic model)
        H = @SMatrix [Dxx Dxy Dxz; Dxy Dyy Dyz; Dxz Dyz Dzz]
        g = @SVector [Dx, Dy, Dz]
        ...
        b = H \ (-g)  # Sub-pixel offset from current integer position

        # If offset is large (|b| > 0.6), move to adjacent INTEGER pixel and re-fit
    ...
    # Compute Laplacian (trace of Hessian) for bright/dark polarity
    laplacian = Dxx + Dyy
