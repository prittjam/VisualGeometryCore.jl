...
- `xi::Int`, `yi::Int`, `zi::Int`: Integer coordinates (x, y, scale index within octave)
- `x::Float64`, `y::Float64`, `z::Float64`: Refined sub-pixel coordinates
- `sigma::Float64`: Scale (sigma) at refined position
- `step::Float64`: Sampling step (2^octave)
...
extremum and moves to the fitted peak location.

# Arguments
- `octave_3d`: 3D array [height, width, depth]
- `x, y, z`: Integer coordinates of discrete extremum
...
- `base_scale`: Base scale parameter
- `step`: Sampling step (2^octave)

# Returns
- `(extremum, converged)`: Refined extremum and convergence flag

# Implementation
Follows VLFeat's `vl_refine_local_extreum_3` (covdet.c:1206-1343)
"""
function refine_extremum_3d(octave_3d::Array{T,3}, x::Int, y::Int, z::Int,
    ...
    for iter in 1:max_iterations
        ...
        dz = (b[3] > 0.6 && z < depth-1 ? 1 : 0) + (b[3] < -0.6 && z > 2 ? -1 : 0)
...
function detect_extrema(response::ScaleSpaceResponse{T};
                       ...
                       octave_resolution::Int=3) where T
